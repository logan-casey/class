...
Nw = 120;  % 120
wgrid = hw.build_wgrid(-2*grid.kbar, 2*grid.kbar, Nw);

% Solve equilibrium
opts = struct();
opts.outer_maxit   = 2; % 30
opts.tol_rtilde    = 1e-3;
opts.outer_verbose = true;

% Damping and caps help stability
...
opts.inner_maxit    = 30;
opts.inner_tol      = 1e-6;
opts.howard_iters   = 50;
opts.improve_every  = 5;
opts.inner_verbose  = false;

eq = hw.solve_equilibrium(wgrid, zgrid, Pz, par, grid, opts);

save('hw_solution.mat', 'eq', 'par', 'grid', 'wgrid', 'zgrid', 'Pz');
